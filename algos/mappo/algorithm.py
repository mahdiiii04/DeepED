    ...
    def __init__(
        ...
        policy_net = nn.Sequential(
            MultiAgentMLP(
                ...
                n_agent_outputs=env.full_observation_spec_unbatched[env.action_key].shape[-1],
    ...
    def update(
        ...
        for _ in range(cfg.train.num_epochs):
            for _ in range(cfg.collector.frames_per_batch // cfg.train.minibatch_size):
                ...
                training_tds[-1].set("grad_norm", grad_norm.mean())

                self.optimizer.step()
                self.optimizer.zero_grad()

        stacked = torch.stack(training_tds)
        return {
            "loss/objective": stacked["loss_objective"].mean().item(),
            "loss/critic":    stacked["loss_critic"].mean().item(),
            "loss/entropy":   stacked["loss_entropy"].mean().item(),
            "grad/norm":      stacked["grad_norm"].mean().item(),
        }
